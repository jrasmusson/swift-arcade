...
struct WeatherService {
    ...
    var resultHandler: ((Result<WeatherModel, ServiceError>) -> Void)?
        ...
        let task = URLSession.shared.dataTask(with: url) { data, response, error in
            ...
            guard error == nil else {
                DispatchQueue.main.async {
                    ...
                    handler(Result.failure(generalError))
                }
                return
            }
            ...
            guard (200...299).contains(httpResponse.statusCode) else {
                DispatchQueue.main.async {
                    guard let handler = resultHandler else { return }
                    handler(Result.failure(ServiceError.network(statusCode: httpResponse.statusCode)))
                }
                return
            }
            ...
            DispatchQueue.main.async {
                guard let handler = resultHandler else { return }
                handler(Result.success(weather))
            }
        }
        task.resume()
    }
    ...
    private func parseJSON(_ weatherData: Data) -> WeatherModel? {
        ...
        guard let decodedData = try? JSONDecoder().decode(WeatherData.self, from: weatherData) else {
            DispatchQueue.main.async {
                guard let handler = resultHandler else { return }
                handler(Result.failure(ServiceError.parsing))
            }
            return nil
        }
